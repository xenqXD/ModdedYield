...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Modded Yield is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Logo.Image = "rbxassetid://80122911709452"
...
reference = (function()
	...
	inviteButton.MouseButton1Click:Connect(function()
		if everyClipboard then
			toClipboard("https://discord.gg/kbtd2rmjYb")
		...
		inviteButton.Text = "Copy Discord Invite Link (https://discord.gg/kbtd2rmjYb)"
	end)
	dragGUI(main)
	main.Parent = ScaledHolder
...
function saves()
    if writefileExploit() and readfileExploit() and jsonAttempts < 10 then
        local readSuccess, out = readfile("MY_FE.iy", true)
        if readSuccess then
            if out ~= nil and tostring(out):gsub("%s", "") ~= "" then
                ...
                if not success then
                    ...
                    writefile("MY_FE.iy", defaults, true)
                    wait()
                    saves()
                end
            else
                writefile("MY_FE.iy", defaults, true)
                wait()
                local dReadSuccess, dOut = readfile("MY_FE.iy", true)
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("MY_FE.iy", HttpService:JSONEncode(update))
	end
end
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Modded Yield Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Modded Yield support server.'}
...
CMDs[#CMDs + 1] = {NAME = 'remotespy / rspy', DESC = 'Opens Simple Spy V3 [FIXED]'}
...
CMDs[#CMDs + 1] = {NAME = 'FakeAFKRonaldsGame', DESC = 'Gives you the AFK status - WARNING: RESETS WHILE ALT-TABBING! -'}
CMDs[#CMDs + 1] = {NAME = 'enableshiftlock / enablesl', DESC = 'fuck devs that delete shiftlock, type this to re-enable it'}
...
CMDs[#CMDs + 1] = {NAME = 'tpunanchored / tpua [player]', DESC = 'Great to fuck around in spermaDEV games'}
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'my_fe.iy' or name == 'my_fe' then
...
addcmd('remotespy',{'rspy'},function(args, speaker)
	...
	loadstring(game:HttpGet("https://raw.githubusercontent.com/xenqXD/ModdedYield/refs/heads/main/simplespy.lua"))()
end)
...
task.spawn(function()
	...
	if success then
		if currentVersion ~= latestVersionInfo.Version then
			notify('Outdated','(dont) Get the new version at infyiff.github.io')
		end
...
if os.date("%m %d") == "04 01" then
	...
	Title.Text = string.gsub(Title.Text, "Modded Yield FE", "fortnite cheeto")
end
print("Thanks for using Modded Yield by xenqXD")
